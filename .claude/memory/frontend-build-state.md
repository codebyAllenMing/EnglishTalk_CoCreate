...
3. **上線前換掉假數據**（`grep "FAKE_"`）。個人資料、Find Monsters、線上狀態、週曆已走 API（2026-09-22）；還假的：
...
- 2026-09-23 手機（iOS Chrome）測試：dev 的 hydration 錯誤 `__gcrremoteframetoken` 是 iOS Chrome 自己塞進 `<html>` 的屬性，不是程式問題；根 layout 的 `<html>` 加了 `suppressHydrationWarning`（只壓這一層）。同一天又撞到它在 `<input>` 塞 `__gcruniqueid`，所以 SSR 會畫的表單欄位（PasswordField、AuthField、HostRoomDialog、SettingsForm、MonsterFilters 的 input / select / textarea）也加了；新加的 SSR 表單欄位照做。
